...
function ReviewPage() {
  ...
  const handleTopicClick = async (topicName) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.getLearnedByTopic(topicName);
      setWords(res.data || []);
    } catch (err) {
      ...
      <div className="w-full min-h-screen bg-sky-50 px-4 py-6 flex flex-col">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-6">
            {topics.map((topic) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium shadow-sm border transition-all text-left ${selectedTopicName === topic.name
        ...
        <div className="flex-1 flex items-center justify-center">
          {isLoading ? (
            ...
            <FlashcardSlider wordList={words} mode="review" />
          ) : selectedTopicName ? (
